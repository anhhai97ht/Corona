...
if ( !function_exists( 'meks_log' ) ):
	function meks_log( $mixed ) {
		...
		$existing = $wp_filesystem->get_contents(  FTC_INSTAGRAM_WIDGET_DIR . 'log' );
		$wp_filesystem->put_contents( FTC_INSTAGRAM_WIDGET_DIR . 'log', $existing. $mixed . PHP_EOL );
	}
endif;
...
if ( ! function_exists( 'ftc_get_js_settings' ) ) :
	function ftc_get_js_settings() {
		$js_settings = array();
...
add_action( 'wp_ajax_ftc_save_business_selected_account', 'ftc_save_business_selected_account' );

if ( ! function_exists( 'ftc_save_business_selected_account' ) ) :
	function ftc_save_business_selected_account() {
        
        $options = get_option( 'ftc_instagram_settings', array() );

        $options['access_token'] = $_POST['access_token'];
        $options['user_id'] = $_POST['user_id'];
        $options['username'] = $_POST['username'];
        $options['name'] = $_POST['name'];
        $options['picture_url'] = $_POST['image'];
