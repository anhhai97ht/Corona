...
class Ftc_Instagrams_Widget extends WP_Widget {
	 ...
	 * @var      string
	 */
	protected $widget_slug = 'ftc_instagram_new';
	 ...
	 * @var      string
	 */
	protected $widget_text_domain = 'ftc-instagram-feed';
	...
	public function __construct() {
		...
		parent::__construct(
			$this->widget_slug,
			__( 'FTC - Instagram Feed', 'ftc-instagram-feed' ),
			array(
				'description' => __( 'Easily display Instagram photos with this widget.', 'ftc-instagram-feed' ),
			)
		);
		...
		$this->defaults = array(
			...
			'link_text'        => __( 'Follow us on Instagram', 'ftc-instagram-feed' ),
			'hover_animation'  => '',
			'is_slider'        => 0,
			'show_nav'         => 1,
			'auto_play'        => 1,
		...
		$widget_settings    = get_option( 'ftc_instagram_settings' );
		...
		$this->defaults = apply_filters( 'ftc_instagram_widget_modify_defaults', $this->defaults );
	...
	public function widget( $args, $instance ) {
		...
		ob_start();
		include $this->get_template( FTC_INSTAGRAM_WIDGET_DIR . 'views/widget_html' );
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['hover_animation']  = strip_tags( $new_instance['hover_animation'] );
	...
	public function form( $instance ) {
		...
		include FTC_INSTAGRAM_WIDGET_DIR . 'views/widget_admin_html.php';
	...
	function generate_transient_key( $usernames_or_hashtags ) {
		...
		$transient_key = md5( 'ftc_instagram_widget_' . $usernames_or_hashtags );
	...
	protected function get_instagram_data_with_token() {
		...
		if ( is_wp_error( $response ) || 200 != wp_remote_retrieve_response_code( $response ) ) {
			$error_message = __('Connection error. Connection fail between instagram and your server. Please try again', 'ftc-instagram-feed');
		...
		if ( empty( $images ) ) {
			return new WP_Error( 'no_images', esc_html__( 'Images not found. This may be a temporary problem. Please try again soon.', 'ftc-instagram-feed' ) );
		}
	...
	protected function get_instagram_data_without_token() {
		...
		if ( empty( $data ) ) {
			return new WP_Error( 'blocked',  sprintf( esc_html__('Instagram has returned empty data. Please authorize your Instagram account in the %s plugin settings %s.', 'ftc-instagram-feed' ), '<a href="'.esc_url( admin_url( 'options-general.php?page=ftc-instagram' ) ).'">', '</a>') );
		}
	...
	public function register_widget_styles() {
		wp_enqueue_style( $this->widget_slug . '-widget-styles', FTC_INSTAGRAM_WIDGET_URL . 'css/widget.css' );
	}
	...
	public function register_admin_script() {
		wp_enqueue_script( $this->widget_slug . '-admin-script', FTC_INSTAGRAM_WIDGET_URL . 'js/admin.js', true, FTC_INSTAGRAM_WIDGET_VER );
		wp_enqueue_style( $this->widget_slug . '-admin-styles', FTC_INSTAGRAM_WIDGET_URL . 'css/admin.css' );
	}

	public function refresh_access_token( $options ) {

		if ( empty( $options ) ) {
			$options = get_option( 'ftc_instagram_settings' );
		}
		...
		if ( is_wp_error( $refresh_token_response ) || isset( $refresh_data->error ) ) {
			$error_message = __('Some problem with connection and with refresh token', 'ftc-instagram-feed');
		...
		update_option( 'ftc_instagram_settings', $options );		
