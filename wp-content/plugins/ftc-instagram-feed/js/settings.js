(function ($) {
	$(document).ready(function () {
		...
		var instagram_button_connect = $('.ftc-instagram-button-connect');
		...
		$('body').on('change', 'input[name=meks_connected_account]', function() {
			var $this = $(this).next();
			$('#ftc-access-token').val($this.data('access_token'));
			$('#ftc-user-id').val($this.data('id'));
			$('#ftc-api-type').val('business');
		...
		$('body').on('click', 'button.meks-connect-account', function(){
			...
			$.ajax({
				url: ftc_js_settings.ajax_url,
				type: 'post',
				data: {
					action: 'ftc_save_business_selected_account',
	...
	*/
	function set_personal_api_info( data ) {
		...
		var token = $('#ftc-access-token');
		var user_id = $('#ftc-user-id');
		var token_expires = $('#ftc-token-expires-in');
		var api_type = $('#ftc-api-type');
	...
	*/
	function set_business_api_info( data ) {
		...
		var token = '';
		var input_token_field = $('#ftc-access-token');
		var url_hash_arr = data.split('&');
		...
		var expires_in = parseInt( 86400 * 60 ) + parseInt(Math.floor(Date.now() / 1000));
		$('#ftc-token-expires-in').val( expires_in );
