...
function ftc_testimonial_shortcode($atts){
	...
	if( $testimonials->have_posts() ){
		...
		?>
		<div class="ftc-sb-testimonial owl-carousel <?php echo esc_attr($text_color_style) ?> <?php echo ($show_nav || $show_dots)?'show-navi':''; ?> <?php echo ($is_slider)?'ftc-slider loading':''; ?>" 
...
function ftc_banner_shortcode( $atts, $content = '' ){
	...
	?>
	<div class="ftc-banner <?php echo esc_attr($unique_class) ?> <?php echo esc_attr($style_effect) ?> <?php echo ($opacity_bg_device)?'opacity-bg-device':'' ?> <?php echo ($responsive_size)?'responsive-size':'' ?> <?php echo esc_attr($position_content) ?>">
...
if( !function_exists('ftc_single_image_shortcode') ){
	function ftc_single_image_shortcode( $atts ){
		...
		<div class="ftc-smooth-image <?php echo esc_attr($style_effect) ?>">
			<?php if( $link != '' ):?>
				<a title="<?php echo esc_attr($link_title) ?>" target="<?php echo esc_attr($target); ?>" class="ftc-smooth" href="<?php echo esc_url($link) ?>" >
					<?php 
					if( $img_url != '' ){
						?>
...
if( !function_exists('ftc_blogs_shortcode') ){
	function ftc_blogs_shortcode( $atts, $content = null){
		...
		if( $posts->have_posts() ):
			...
			$atts = compact('title', 'columns', 'categories', 'per_page', 'orderby',
				'order','style', 'show_title', 'show_thumbnail', 'show_author'
				,'show_date', 'show_comment', 'show_excerpt', 'show_readmore',
				'excerpt_words','is_slider', 'show_nav', 'auto_play', 'margin', 'is_masonry', 'show_load_more', 'is_grid',
	...
	if( !function_exists('ftc_products_slider_shortcode') ){
		function ftc_products_slider_shortcode($atts, $content){
			...
			if($product_style = 'default') {
				$add_class_new = "default";
			}  
			if($product_style = 'style_1') {
				$add_class_new = "style_1";
			}  

			if( $products->have_posts() ): 
				?>              
				<div class="<?php echo esc_attr(implode(' ', $classes)); ?> <?php echo $add_class_new ?> " data-slider="<?php echo esc_attr((int)$is_slider) ?>" data-columns="<?php echo esc_attr($columns) ?>" data-autoplay="0"
