...
class Ftc_Metaboxes{
	...
	function add_meta_boxes(){
		$datas = array(
					array(
						'id' 			=> 'page_options'
						,'label' 		=> esc_html__('Page Options', 'corona')
						,'post_type'	=> 'page'
					)
					,array(
						'id'			=> 'testimonial_options'
						,'label'		=> esc_html__('Testimonial Options', 'corona')
						,'post_type'	=> 'ftc_testimonial'
					)
					,array(
						'id'			=> 'team_options'
						,'label'		=> esc_html__('Member Information', 'corona')
						,'post_type'	=> 'ftc_team'
					)
					,array(
						'id'			=> 'portfolio_options'
						,'label'		=> esc_html__('Portfolio Options', 'corona')
						,'post_type'	=> 'ftc_portfolio'
					)
					,array(
						'id'			=> 'portfolio_gallery'
						,'label'		=> esc_html__('Portfolio Gallery', 'corona')
						,'post_type'	=> 'ftc_portfolio'
						,'context'		=> 'side'
						,'priority'		=> 'low'
					)
					,array(
						'id'			=> 'logo_options'
						,'label'		=> esc_html__('Logo Options', 'corona')
						,'post_type'	=> 'ftc_brand'
					)
					,array(
						'id'			=> 'product_options'
						,'label'		=> esc_html__('Product Options', 'corona')
						,'post_type'	=> 'product'
					)
					,array(
						'id'			=> 'post_options'
						,'label'		=> esc_html__('Post Options', 'corona')
						,'post_type'	=> 'post'
					)
					,array(
						'id'			=> 'post_gallery'
						,'label'		=> esc_html__('Post Gallery', 'corona')
						,'post_type'	=> 'post'
						,'context'		=> 'side'
						,'priority'		=> 'low'
					)
					...
					,array(
						'id'			=> 'product_360'
						,'label'		=> esc_html__('Product 360', 'corona')
						,'post_type'	=> 'product'
						,'context'		=> 'side'
						,'priority'		=> 'low'
					)
			...
			switch( $option['type'] ){
				case 'text':
					$html .= '<div class="ftc-meta-box-field">';
						...
						$html .= '<div class="field">';
							$html .= '<input type="text" name="ftc_'.$option['id'].'" id="ftc_'.$option['id'].'" value="'.esc_attr($post_meta_value).'" />';
					...
					$html .= '<div class="ftc-meta-box-field">';
						...
						$html .= '<div class="field">';
							...
							foreach( $option['options'] as $key => $value ){
								$html .= '<option value="'.$key.'" '.selected($key, $post_meta_value, false).'>'.esc_attr($value).'</option>';
							}
							$html .= '</select>';
							if( strlen($option['desc']) > 0 ){
								$html .= '<p class="description">'.$option['desc'].'</p>';
				...
				case 'textarea':
					$html .= '<div class="ftc-meta-box-field">';
						$html .= '<label for="ftc_'.$option['id'].'">'.$option['label'].'</label>';
						$html .= '<div class="field">';
							$html .= '<textarea name="ftc_'.$option['id'].'" id="ftc_'.$option['id'].'">'.esc_attr($post_meta_value).'</textarea>';
						if( strlen($option['desc']) > 0 ){
							$html .= '<p class="description">'.$option['desc'].'</p>';
						}
						$html .= '</div>';
				...
				case 'upload':
					...
					$html .= '<div class="ftc-meta-box-field">';
						$html .= '<label for="ftc_'.$option['id'].'">'.$option['label'].'</label>';
						$html .= '<div class="field">';
							$html .= '<input type="text" class="upload_field" name="ftc_'.$option['id'].'" id="ftc_'.$option['id'].'" value="'.esc_url($post_meta_value).'" />';
							$html .= '<input type="button" class="ftc_meta_box_upload_button" value="'.esc_html__('Select Image', 'corona').'" />';
							$html .= '<input type="button" class="ftc_meta_box_clear_image_button" value="'.esc_html__('Clear Image', 'corona').'" '.($post_meta_value?'':'disabled').' />';
						if( strlen($option['desc']) > 0 ){
							$html .= '<p class="description">'.$option['desc'].'</p>';
						}
						if( $post_meta_value ){
				...
				case 'gallery':
					...
					$html .= '<div class="ftc-meta-box-field ftc-gallery-box">';
						...
						$html .= '</ul>';
						$html .= '<input type="hidden" class="meta-value" name="ftc_'.$option['id'].'" id="ftc_'.$option['id'].'" value="'.esc_url($post_meta_value).'" />';
						$html .= '<a href="#" class="add-image">'.esc_html__('Add Images', 'corona').'</a>';
					$html .= '</div>';
				break;
				...
				case 'colorpicker':
					$html .= '<div class="ftc-meta-box-field">';
						$html .= '<label for="ftc_'.$option['id'].'">'.$option['label'].'</label>';
						$html .= '<div class="field">';
							$html .= '<input type="text" class="colorpicker" name="ftc_'.$option['id'].'" id="ftc_'.$option['id'].'" value="'.esc_attr($post_meta_value).'" />';
							if( strlen($option['desc']) > 0 ){
								$html .= '<p class="description">'.$option['desc'].'</p>';
							}
						$html .= '</div>';
